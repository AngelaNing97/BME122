...
namespace DataStructures
{
    ...
    {
        // An object of the "Node" class is one node in a linked list of nodes.
        // Each node holds a link to the node which follows it in the linked list.
        // Each node holds one data item of type "Drug".
        partial class Node
        {
            Node next;
            Drug data;
        ...
        //why do we need to specify linkedlist
        //past in testing method
        public LinkedList RemoveAll(Func<Drug,bool> test)
        {
            ...
            Node previous = null;
            //Drug data;
            LinkedList vitaminList=new LinkedList();
			...
			while(current != null)
			{
				...
				if(test.Invoke(current.Data)==true)
				{
                    //case where there is only one element
                    if(count==1)
                    {
                        //moving current to point to the next node
                        current = current.Next;
                        
                        //old points to the object that head points to
                        old=head;
                        //unlink THE node
                        head=null;
                        tail=null;
                    ...
                    //case where the first match is at the head
                    ...
                    //case where the match is at the tail
                    ...
                    else //case where match is at the middle
                    {
                        old=current;
...
namespace Bme121.Pa4
{
    ...
    static partial class Program
    {
        ...
        static void Main( )
        {
         //read the Drug record from the file
         ...
         foreach( Drug d in drugRecords.DrugList )
         {
             drugList.Append(d);
         }
         
         LinkedList removed = drugList.RemoveAll( IsVitamin );
         WriteLine("removing at middle");
         
         foreach( Drug d in removed )
         {
             WriteLine(d);
         }
         
         
         // test case 2:all matches
         LinkedList allMatchList= new LinkedList();
         allMatchList= removed;
         
         removed=allMatchList.RemoveAll(IsVitamin);
         WriteLine("test case: all matches");
         foreach( Drug d in removed)
         {
             WriteLine(d);
         }
         ...
         LinkedList noMatchList=new LinkedList();
         noMatchList=drugList;
         removed=drugList.RemoveAll(IsVitamin);
         WriteLine("test case: No match");
         foreach( Drug d in removed)
         {
             WriteLine(d);
         }
         ...
         removed = emptyList.RemoveAll( IsVitamin );
         WriteLine("test case: empty list");
         foreach( Drug d in removed )
         {
             WriteLine(d);
         }
         
         //test case 5: one at head

         DrugRecords oneAtHeadRecords=new DrugRecords("RXQT1503-101.txt"); //this is a modified file that contains one vitamin at head
         
         //copy the drug records into a linked list
         
         drugList=new LinkedList();   
         foreach( Drug d in oneAtHeadRecords.DrugList )
         {
             drugList.Append(d);
         }
         
          removed = drugList.RemoveAll( IsVitamin );
         WriteLine("test case: one at head");
         foreach( Drug d in removed )
         {
             WriteLine(d);
         }
         
         //test case 6: one at tail
         
         DrugRecords oneAtTailRecords=new DrugRecords("RXQT1503-102.txt"); //this is a modified file that contains one vitamin at tail
         
         drugList=new LinkedList();   
         foreach( Drug d in oneAtTailRecords.DrugList )
         {
             drugList.Append(d);
         }
         
          removed = drugList.RemoveAll( IsVitamin );
         WriteLine("test case: one at tail");
         foreach( Drug d in removed )
         {
             WriteLine(d);
         }
        ...
        //testing whether the name of drug contains vitamin
        static bool IsVitamin(Drug d)
        {
          if(d.Name.Contains("VITAMIN"))
          { return true;
          }else{ return false; }
        }
